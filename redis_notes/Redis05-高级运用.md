...
下次再查的时候，先命中这个key，就不走布隆过滤器，直接返回了。也可以个这个key加一个很短的过期时间，比如5min  
...
Redis当做缓存用，数据可以丢；做数据库用，数据不能丢。但是Redis数据库 + MySQL会带来数据一致性的问题，因为内存是ns级的，MySQL写磁盘，是ms级的，差了10万倍。一般如果拿Redis做数据库的话，不用强一致性，不要试图让MySQL追平这个一致性，
中间加一个消息队列，异步往MySQL里面去写。Redis做数据库的书，在速度之外还要加上持久性，内存里的东西掉电易失。下一节讲持久化  
