...
注意make完 不要make install, 先要停止当前运行着的nginx：首先停止keepalived：```service keepalived stop``` 然后再停止nginx：```service nginx stop```或者杀进程;
启动的时候先启动nginx，再启动keepalived

拷贝objs下的 nginx文件到 已安装好的sbin目录下 覆盖即可

```shell
cp ./nginx /usr/local/nginx/sbin/
```

其实执行configure命令之后objs目录已经生成，但是其中还没有nginx可执行文件，只有make之后才有
...
192.168.1.120 和 192.168.1.121 上的nginx进程是主备关系；192.168.1.120 和 192.168.1.121 上的trackerd进程是主主关系，由客户端得到tracker的列表，再选择tracker的机器  

查看nginx插件的命令：```/usr/local/nginx/sbin/nginx -V```
